                                ...
                                pitchingData?.map( (item) => {
                                    return (
                                        <div className='player-link'>
                                            ...
                                            <a href={'http://daseball.adaptable.app/player/'+item.name} value={item._id} key={item.name}>{ item.name }</a><span>{ "★".repeat(Math.max(0, (Math.floor((item.praying + item.publicity + item.pope) / 100)))) + "☆".repeat(Math.max(0, ((Math.round((item.praying + item.publicity + item.pope) / 100)) - (Math.floor((item.praying + item.publicity + item.pope) / 100))))) }</span>
                                            </div>
                                        </div>
                                    )
                                })
                            ...
                            <h2>Batting Rotation</h2>
                            {
                                ...
                                battingData?.map( (item) => {
                                    return (
                                        <div className='player-link'>
                                            <div className='split-para'>
                                            <a href={'http://daseball.adaptable.app/player/'+item.name} value={item._id} key={item.name}>{ item.name }</a><span>{ "★".repeat(Math.max(0, (Math.floor((item.battery + item.assault + item.resistingArrest) / 100)))) + "☆".repeat(Math.max(0, ((Math.round((item.battery + item.assault + item.resistingArrest) / 100)))) - (Math.floor(Math.max(0, ((item.battery + item.assault + item.resistingArrest) / 100))))) }</span>
                                            </div>
                                        </div>
                                    )
                                })
                            ...
                            <h2>Pocket Rotation</h2>
                            {
                                ...
                                pocketData?.map( (item) => {
                                    ...
                                    return (
                                        <div className='player-link'>
                                            <div className='split-para'>
                                            <a href={'http://daseball.adaptable.app/player/'+item.name} value={item._id} key={item.name}>{ item.name }</a><span>{"★".repeat(Math.max(0, Math.floor(statstotal / 400))) + (statstotal % 400 >= 200 ? '☆' : '')}</span>
                                            </div>
                                        </div>
                                    )
                                })
